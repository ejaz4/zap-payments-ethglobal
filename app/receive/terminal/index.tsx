...
export default function CreateTerminalPaymentRequestScreen() {
  ...
  // Terminal IP state (default cosmetic placeholder)
  const [terminalIp, setTerminalIp] = useState("192.168.12.3");
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      console.log(
        "[CreateTerminalRequest] Payment request created with hash:",
        hash,
      );

      // Log the NFC/QR payload that would be sent to terminal
      const nfcPayload = JSON.stringify({
        chainId: selectedChainId.toString(),
        address: contractAddress,
        network: "ethereum",
      });
      console.log("[CreateTerminalRequest] Terminal NFC Payload:", nfcPayload);
      console.log(
        "[CreateTerminalRequest] Terminal IP:",
        terminalIp || "Not specified",
      );

      // Haptic feedback
      Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);

      // Create local request record with terminal mode flag
      ...
      const request: PaymentRequest = {
        ...
        createdAt: Date.now(),
        txHash: hash,
        // Add terminal info (stored in description or extra field)
        terminalMode: true,
        terminalIp: terminalIp || undefined,
      } as PaymentRequest & { terminalMode?: boolean; terminalIp?: string };

      // Set as active request
      setActiveRequest(request);

      // Navigate to terminal status screen
      router.replace("/receive/terminal/status");
    } catch (err: any) {
      console.error("Failed to create terminal payment request:", err);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Zap Terminal</Text>
        <View style={{ width: 24 }} />
      </View>
      ...
      >
        ...
        >
          ...
          {/* Terminal IP Section */}
          <View style={[styles.section, styles.terminalSection]}>
            <View style={styles.terminalHeader}>
              <Ionicons
                ...
                color="#8B5CF6"
              />
              <Text style={styles.terminalTitle}>Zap Terminal Settings</Text>
            </View>
            <View style={styles.inputGroup}>
              ...
              />
              <Text style={styles.helperText}>
                Enter the IP address of your Zap Terminal device (optional for
                now)
              </Text>
            </View>
          </View>
        ...
        <View style={styles.footer}>
          <Button
            title={isSubmitting ? "Creating..." : "Create Request on Terminal"}
